...
.btn-sync { padding:6px 13px; border-radius:20px; border:1px solid var(--border); background:var(--bg3); color:var(--text2); font-size:0.75rem; font-weight:500; cursor:pointer; font-family:inherit; }
...
.tab { padding:6px 15px; border-radius:20px; font-size:0.82rem; font-weight:500; cursor:pointer; border:1px solid var(--border); color:var(--text2); background:var(--bg4); white-space:nowrap; }
...
.gname { width:100%; background:transparent; border:none; font-size:1.05rem; font-weight:600; color:var(--text1); font-family:inherit; }
.gname:focus { outline:none; }
.sep { height:1px; background:var(--border); }
.add-row { display:flex; gap:8px; padding:12px 16px; }
.add-inp { flex:1; background:var(--bg3); border:1px solid var(--border); border-radius:var(--radius-s); padding:9px 13px; color:var(--text1); font-size:0.88rem; font-family:inherit; }
.add-inp:focus { outline:none; border-color:var(--accent); }
.add-inp::placeholder { color:var(--text3); }
.btn-add { padding:9px 16px; border-radius:var(--radius-s); border:none; background:var(--accent); color:#fff; font-size:0.85rem; font-weight:600; cursor:pointer; font-family:inherit; }
.btn-add:active { opacity:.7; }
.tbl-head { display:grid; grid-template-columns:48px 52px 72px 36px 58px 58px 26px; gap:3px; padding:8px 12px; border-top:1px solid var(--border); }
.tbl-head span { font-size:0.58rem; color:var(--text3); font-weight:600; letter-spacing:0.04em; text-transform:uppercase; overflow:hidden; }
.tbl-head span:nth-child(2),
.tbl-head span:nth-child(4),
.tbl-head span:nth-child(5),
.tbl-head span:nth-child(6) { text-align:right; }
.srow { display:grid; grid-template-columns:48px 52px 72px 36px 58px 58px 26px; gap:3px; align-items:center; padding:11px 12px; border-top:1px solid var(--border); }
.srow:active { background:rgba(255,255,255,0.03); }
.c-id    { font-size:0.85rem; font-weight:600; color:var(--text1); overflow:hidden; }
.c-price { font-size:0.82rem; font-weight:500; color:var(--text1); text-align:right; }
.c-sig   { font-size:0.75rem; font-weight:600; display:flex; align-items:center; gap:3px; overflow:hidden; }
.c-days  { font-size:0.72rem; color:var(--text2); text-align:right; }
.c-lv    { font-size:0.78rem; text-align:right; font-weight:500; }
.c-lv.resist  { color:#ff6b6b; }
.c-lv.supprt  { color:#5ec75e; }
.c-lv.newhigh { color:#ff9f0a; font-size:0.68rem; }
.c-lv.newlow  { color:#64d2ff; font-size:0.68rem; }
...
.btn-scan { padding:8px 18px; border-radius:20px; border:none; background:var(--green); color:#000; font-size:0.82rem; font-weight:700; cursor:pointer; font-family:inherit; display:flex; align-items:center; gap:5px; }
...
function renderRows(stocks,gi){
  ...
  return stocks.map(function(s,si){
    ...
    var resCls,supCls,resVal,supVal;
    if(s.resistance!=null){ resVal=s.resistance; resCls="c-lv resist"; }
    else{ resVal="新高"; resCls="c-lv newhigh"; }
    if(s.support!=null){ supVal=s.support; supCls="c-lv supprt"; }
    else{ supVal="新低"; supCls="c-lv newlow"; }
    ...
    return "<div class=\\"srow\\">"+
      ...
      "<span class=\\""+resCls+"\\">"+resVal+"</span>"+
      "<span class=\\""+supCls+"\\">"+supVal+"</span>"+
